document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("scroll", () => {
    ...
    document.querySelectorAll("section").forEach((section) => {
      ...
      if (
        ...
        document
          .querySelectorAll(".navbar a, .nav .nav-link")
          .forEach((link) => {
  ...
  document
    .querySelectorAll(".project-image img, .logos-row img, .logos img")
    .forEach((img) => {
...
document.addEventListener("scroll", function () {
  const elements = document.querySelectorAll(
    ".tech-stack span, .about-container h4, .bio",
  );
...
if (canvas) {
  ...
  class Particle {
    constructor() {
      ...
      this.color = `rgba(0, 240, 255, ${Math.random() * 0.5 + 0.3})`;
    }
    update() {
      this.x += this.speedX;
      this.y += this.speedY;
  ...
  function initParticles() {
    particlesArray = [];
    const numberOfParticles = Math.min(80, (canvas.width * canvas.height) / 15000);
    ...
    for (let a = 0; a < particlesArray.length; a++) {
      for (let b = a; b < particlesArray.length; b++) {
        ...
        if (distance < 15000) {
          ...
          ctx.strokeStyle = `rgba(112, 0, 255, ${opacityValue * 0.4})`;
          ctx.lineWidth = 1;
          ctx.beginPath();
